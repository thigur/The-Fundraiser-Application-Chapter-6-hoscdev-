...
contract("Fundraiser", accounts => {
   ...
   describe("setBeneficiary", () => {
      ...
      it("throws an error when called from a non-owner account", async () => {
         try{
            ...
            assert.fail("withdraw as not restriced to owners")
            } catch(err) {
   ...
   describe("making donations", () => {
      ...
      it("increases the totalDonations amount", async () => {
         ...
         assert.equal(diff, value, "difference shoudL match the donation value");
      });
   ...
   describe("access control", () => {
      it("throws an error when called froma non-owner account", async () => {
         try{
      ...
      it("permits the owner to call the function", async () => {
         try{
            ...
            assert(true, "no errors wer thrwon");
         } catch(err) {
      ...
      it("transfers balance to beneficiary", async () => {
         ...
         const currentBeneficiarytBalance = await web3.eth.getBalance(beneficiary);

         await fundraiser.withdraw({from: owner});

         const newContractBalance = await web3.eth.getBalance(fundraiser.address);
         const newBeneficiarytBalance = await web3.eth.getBalance(beneficiary);
         const beneficiaryDifference = newBeneficiarytBalance - currentBeneficiarytBalance;

        assert.equal(newContractBalance, 0, "contract should have a 0 balance");
        assert.equal(beneficiaryDifference,
     ...
     describe("fallback function", () => {
        const value = web3.utils.toWei('0.0289');
         ...
         it("increases donationsCount", async () => {
           ...
           const nwDonationsCount = await fundraiser.donationsCount();

          assert.equal(1, nwDonationsCount - currentDonationsCount, 
                       "donation should increment by 1");
         });//it()
